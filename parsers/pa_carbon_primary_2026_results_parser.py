#!/usr/bin/env python3
"""Parse Carbon County PA 2026 Primary precinct results.

Source: Carbon County StatementOfVotesCastRPT Final Official 060226.pdf
(SOVC crosstab with rotated column headers — same shape as Bedford's 2026
primary report. Contest list: Governor, Lieutenant Governor,
Representative in Congress 7th District, Representative in the General
Assembly 122nd District, and per-precinct Committee Person races.

Adapted from pa_carbon_primary_2026_results_parser.py with Carbon's
county name, committee contest name, and county-total markers.

Usage:
    python parsers/pa_carbon_primary_2026_results_parser.py <input.pdf> <output.csv>
"""
...
HEADER_RE = re.compile(
    ...
    r"|(?:Democratic|Republican) Committee Person .+?)"
...
def parse_contest(pdf, start_page: int, end_page: int, office_raw: str, party: str
    ...
    for i in range(start_page, last):
        ...
        for tbl in tables:
            ...
            for row in tbl[1:]:
                ...
                if upper_label in ("CARBON COUNTY", "PAGE") or upper_label.startswith("PAGE"):
                    continue
                if label in VOTE_METHODS:
                    continue
                if "CARBON COUNTY - TOTAL" in upper_label or "COUNTY - TOTAL" in upper_label:
                    continue
                if upper_label.startswith("CUMULATIVE"):
                    continue
                precinct = clean_precinct(label)
                if precinct not in candidate_votes:
                    candidate_votes[precinct] = {}
                ...
                for ci, cname in col_candidates:
                    ...
                    if final_name not in candidate_votes[precinct]:
                        candidate_votes[precinct][final_name] = 0
                    if final_name not in candidate_order:
                        candidate_order.append(final_name)
                    candidate_votes[precinct][final_name] += votes
    ...
    for precinct in precinct_order:
        ...
        for cand in order:
            out.append({
                "county": "Carbon", "precinct": precinct,
...
def parse_carbon(pdf_path: Path) -> list[dict]:
    rows: list[dict] = []
