#!/usr/bin/env python3
"""
Generic 2026 PA primary precinct parser for Electionware counties.
...
report-level ``skip_prefixes`` for the 2026 primary ("General Primary" /
"GENERAL PRIMARY ELECTION" instead of "Municipal Election", "May 19, 2026"
instead of "November 4, 2025"). The party-on-office-header parsing is handled
by the shared ``electionware_primary_np`` engine.

Usage:
    python parsers/pa_electionware_primary_2026.py <County> <input.pdf> <output.csv>

where <County> matches the name suffix of an existing
``parsers/pa_<county_lower>_general_2025_results_parser.py`` module
(e.g. ``Beaver``, ``Berks``, ``Butler``, ``Centre``, ``Chester``,
``Clearfield``, ``Clinton``).

Counties whose 2026 primary source PDF does not match the Electionware
...
def _adapt_skip_prefixes(skip_prefixes: tuple[str, ...]) -> tuple[str, ...]:
    """Translate 2025-general skip_prefixes to 2026-primary equivalents.
    ...
    for p in skip_prefixes:
        # Election name variants used by PA counties in 2026.
        p2 = p.replace("Municipal Election", "General Primary")
        p2 = p2.replace("MUNICIPAL ELECTION", "GENERAL PRIMARY ELECTION")
        ...
        for old_date in (
            ...
            p2 = p2.replace(old_date, "May 19, 2026")
        adapted.append(p2)
    # Always include the canonical primary header lines (dedup). Cover both
    # the title-case ("General Primary") and upper-case
    # ("GENERAL PRIMARY ELECTION") report-banner variants seen across counties.
    canonical = (
